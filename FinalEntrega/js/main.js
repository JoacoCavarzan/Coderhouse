...
function validationForm(turnObject) {
  ...
  let turnsArrayForValidation =
    JSON.parse(localStorage.getItem("turnsData")) || [];
...
function deleteTurnObject(turnID) {
  ...
  let turnIndexFromArray = turnsArrayFromStorage.findIndex(
    (element) => element.turnID === turnID
  );
...
function insertRowInTurnTable(turnObject) {
  ...
  editButton.addEventListener("click", (e) => {
    ...
    let modalContainerEdit = document.querySelectorAll(
      ".modalContainerEdit"
    )[0];
...
function editTurn() {
  ...
  //Busco el ID que coincida
  let turnIndexFromArray = turnsArrayFromStorage.findIndex(
    (element) => element.turnID == turnIdEdit
  );
  //Modifico el array con los nuevos valores
  let eliminado = turnsArrayFromStorage.splice(
    turnIndexFromArray,
    1,
    turnEditObject
  );
...
function loadArrayFromStorage() {
  ...
  if (
    newTurnsArrayFromStorage != null ||
    newTurnsArrayFromStorage != undefined
  ) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  turnsArrayFromStorage.forEach(
    (e) => (e.turnDate = new Date(`${e.turnDate}T00:00:00`))
  );
  //Guardo el array que se filtra con el rango horario
  let arrayFilterMonth = turnsArrayFromStorage.filter(
    (e) => e.turnDate.getMonth() + 1 == actualMonth
  );
...
btnFilterHour.addEventListener("click", (e) => {
  ...
  let arrayFilterHour = turnsArrayFromStorage.filter(
    (e) =>
      e.turnTime >= formFilterHourStart &&
      e.turnTime <= formFilterHourEnd &&
      e.turnDate.getMonth() + 1 == actualDate.getMonth() + 1
  );
...
btnFilterDate.addEventListener("click", (e) => {
  ...
  let arrayFormFilterDay = turnsArrayFromStorage.filter(
    (e) =>
      e.turnDate.getDate() == formFilterDay &&
      e.turnDate.getMonth() + 1 == actualDate.getMonth() + 1
  );
